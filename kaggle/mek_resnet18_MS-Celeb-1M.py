"""
MEK + ResNet-18 with MS-Celeb-1M face backbone — single-file Kaggle script (FER-2013 | RAF-DB).

AUTHOR-REPRODUCTION variant. Same MEK method as kaggle/mek_resnet18.py, but with the
paper's EXACT ResNet recipe (Adam @ 1e-4, ExponentialLR γ=0.9, ε=0.1, λ_flip=2, 60
epochs on RAF-DB) AND an MS-Celeb-1M face-recognition pretrained backbone instead of
ImageNet — the setup behind the paper's RAF-DB ResNet-18 = 89.77% acc / 82.44%
mean-class. Add the checkpoint (resnet18_msceleb.pth, e.g. from the RUL repo) as a
Kaggle dataset and point FACE_WEIGHTS_PATH at it below; if it is missing the run falls
back to ImageNet (becoming the baseline), so it never hard-fails.
...
CKPT_PATH = f"mek_{ARCH}_{DATASET}_msceleb_best.pth"
