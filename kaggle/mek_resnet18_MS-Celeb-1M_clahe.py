"""
MEK + ResNet-18 (MS-Celeb-1M) + CLAHE — single-file Kaggle script (FER-2013 | RAF-DB).

AUTHOR-REPRODUCTION variant WITH CLAHE. Identical to kaggle/mek_resnet18_MS-Celeb-1M.py
(paper recipe: Adam @ 1e-4, ExponentialLR γ=0.9, ε=0.1, λ_flip=2, 60 epochs on RAF-DB;
MS-Celeb-1M backbone) but it ALSO applies CLAHE lighting normalization in both the train
and eval transforms — the same CLAHEEqualize demo.py uses. So the model is trained on the
CLAHE-normalized distribution the live demo feeds it (train/serve match), letting you run
demo.py with "CLAHE" ON. Unlike the webcam script it keeps the light reproduction
augmentation, so it stays close to the paper's accuracy.

Add the checkpoint (resnet18_msceleb.pth, e.g. from the RUL repo) as a Kaggle dataset and
point FACE_WEIGHTS_PATH at it below; if it is missing the run falls back to ImageNet, so it
never hard-fails. Needs opencv (cv2) for CLAHE — preinstalled on Kaggle; if absent the
CLAHE step is a no-op.

Method: Mine Extra Knowledge (Zhang et al., NeurIPS 2023, arxiv 2310.19636), with the
same two losses:
...
# User toggles
# ════════════════════════════════════════════════════════════════════
DATASET     = "rafdb"        # "rafdb" (the tuned MS-Celeb best) or "fer2013"
...
# MS-Celeb-1M face-recognition backbone — this is what makes this file the reproduction
# (kaggle/mek_resnet18.py keeps the ImageNet baseline). Add resnet18_msceleb.pth as a
# Kaggle dataset and set FACE_WEIGHTS_PATH. The loader tolerates key-naming differences
# and FALLS BACK to ImageNet if the file is missing/unmatched, so the run never hard-fails.
USE_FACE_BACKBONE = True
FACE_WEIGHTS_PATH = "/kaggle/input/resnet18-msceleb/resnet18_msceleb.pth"
# A .pth (recommended: resnet18_msceleb.pth) loads safely with weights_only=True and
# needs NO hash — leave this empty. It is MANDATORY only if you load a .pkl instead:
...
if DATASET == "fer2013":
    ...
    BATCH_SIZE         = 64
    EPOCHS             = 80
    LR                 = 1e-4         # MS-Celeb on FER not separately tuned — paper default
    ROTATION_DEG       = 10
    COLOR_JITTER       = 0.2
    ...
    BATCH_SIZE         = 64
    EPOCHS             = 60
    LR                 = 3e-4         # validation-tuned best (MS-Celeb MEK-RN18, RAF-DB) — see results-CV-project.md
    ROTATION_DEG       = 15
    COLOR_JITTER       = 0.3
...
# Shared MEK hyperparameters — paper's ResNet recipe (ε=0.1, λ_flip=2, Adam +
# ExponentialLR γ=0.9). LR is validation-tuned per dataset above. NB: the original
# *repo* (Swin-T) used ε=0.3, λ_flip=0.1; the paper's ResNet experiments
# (RN18 = 89.77% / RN50 = 90.29% on RAF-DB) use ε=0.1 and λ=2.
DROPOUT             = ARCH_DROPOUT
LABEL_SMOOTH        = 0.1
FLIP_LOSS_WEIGHT    = 2.0
MOMENTUM            = 0.9
WEIGHT_DECAY        = 1e-4
GRAD_CLIP           = 5.0
VAL_SPLIT           = 0.1
...
CKPT_PATH = f"mek_{ARCH}_{DATASET}_msceleb_clahe_best.pth"
...
class CLAHEEqualize:
    ...
    trains on the same lighting-normalized distribution the demo feeds it. No-op if
    OpenCV is missing."""
...
def _load_state_any(path):
    """Read a face-recognition checkpoint: a torch .pth/.pt or a numpy-dict .pkl.

    Security: .pth/.pt uses weights_only=True (safe). A .pkl is unpickled (arbitrary-
    code risk) ONLY if its SHA-256 matches the source-set FACE_WEIGHTS_SHA256 — fails
    closed otherwise. The recommended resnet18_msceleb.pth takes the safe .pth path."""
...
wb = _init_wandb(f"mek_{ARCH}_{DATASET}", {
    ...
    "flip_loss_weight": FLIP_LOSS_WEIGHT, "optimizer": "Adam", "backbone": BACKBONE,
})
