"""
Webcam-DEPLOY MEK + ResNet-18 (MS-Celeb-1M) + CLAHE + OpenCV face-crop — single-file Kaggle script.

The full webcam-deployment recipe. Same MEK method (Re-balanced Smooth Label +
Re-balanced Attention Consistency) and MS-Celeb-1M face backbone as the webcam script,
but the TRAIN/EVAL transforms now match demo.py's live preprocessing EXACTLY: OpenCV
Haar face detect&crop (0.2 margin) THEN CLAHE lighting normalization — so the deployed
model trains on precisely the distribution the live demo feeds it (train/serve match on
BOTH the face crop and the lighting). Run demo.py with "Detect & crop face" AND "CLAHE"
both ON. Tuned for a LIVE webcam rather than the last point of benchmark accuracy:
...
Kaggle dataset and point FACE_WEIGHTS_PATH at it (falls back to ImageNet if missing).
  ...
  • OpenCV Haar face detect&crop + CLAHE applied in BOTH train and eval (USE_FACE_CROP /
    USE_CLAHE) — a byte-for-byte copy of demo.py's detect_face + apply_clahe, in the same
    order, so there is no train/serve gap. Both no-op gracefully if OpenCV is unavailable
    or no face is found (datasets are already tight face crops; webcam frames are not).
...
which demo.py auto-discovers (the crop/CLAHE are demo-time toggles, model is unchanged).
...
if DATASET == "fer2013":
    ...
    DATA_ROOT, BASE_LR, EPOCHS = RAF_ROOT, 1e-3, 80   # validation-tuned best: 80 ep (was 60)
    ROTATION_DEG = 15
else:
...
USE_FACE_CROP       = True        # apply demo.py's OpenCV Haar face detect&crop too (before CLAHE)
...
# ════════════════════════════════════════════════════════════════════
class FaceCrop:
    """Crop the largest Haar-cascade face with a 0.2 margin — a byte-for-byte copy of
    demo.py's detect_face (same cascade, scaleFactor=1.1, minNeighbors=5, minSize=(48,48),
    margin=0.2*max(w,h)). No-op if OpenCV is missing or no face is found (the datasets are
    already tight crops; live webcam frames are not — this closes that train/serve gap).
    The cascade is built once and cached (loading the XML per-sample would be far too slow)."""
    _cascade = None
    def __call__(self, img):
        try:
            ...
            return img
        if FaceCrop._cascade is None:
            FaceCrop._cascade = cv2.CascadeClassifier(
                cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
        rgb = np.array(img.convert("RGB"))
        gray = cv2.cvtColor(rgb, cv2.COLOR_RGB2GRAY)
        faces = FaceCrop._cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5,
                                                   minSize=(48, 48))
        if len(faces) == 0:
            return img
...
# demo.py order is detect_face THEN clahe — keep it identical here.
_pre = ([FaceCrop()] if USE_FACE_CROP else []) + ([CLAHEEqualize()] if USE_CLAHE else [])
...
eval_transform = transforms.Compose(_pre + [
    transforms.Resize((IMG_SIZE, IMG_SIZE)),
...
@torch.no_grad()
def evaluate(loader, eval_model=None, return_per_class=False):
    # Default to the global `model` (restored to best EMA weights after training) so the
    # tuning harness's metrics block can call evaluate(val_loader, return_per_class=True).
