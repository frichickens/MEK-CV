"""
ResNet-18 + (FER2013 | RAF-DB) — single-file Kaggle training script.
...
ResNet-18-specific tuning: lr_mult=1.0 (the baseline), dropout=0.40.
"""
...
ARCH_LR_MULT    = 1.0          # baseline — deeper backbones lower this
ARCH_DROPOUT    = 0.40
...
if DATASET == "fer2013":
    ...
    BATCH_SIZE         = 128
    EPOCHS             = 100
    BASE_LR            = 1.4e-2        # validation-tuned best (lr14e3-wd1e4); see results-CV-project.md
    LABEL_SMOOTH       = 0.1
    MIXUP_ALPHA        = 0.2
    ROTATION_DEG       = 10
    COLOR_JITTER       = 0.2
    ...
    TEST_DIR           = f"{DATA_ROOT}/test"
    NUM_CLASSES        = 7
    IMG_SIZE           = 224       # upscale clean 100×100 faces to use full pretrained backbone
    CROP_SIZE          = 224
    CROP_PADDING       = 8
    BATCH_SIZE         = 64
    EPOCHS             = 80         # validation-tuned best (80 ep > default 60)
    BASE_LR            = 3e-3       # validation-tuned best (lr3e3); see results-CV-project.md
    LABEL_SMOOTH       = 0.05
    MIXUP_ALPHA        = 0.0       # mixup helps less on cleaner datasets
    ROTATION_DEG       = 15
    COLOR_JITTER       = 0.3
...
# ImageNet stats — required because we use ImageNet-pretrained weights.
...
# Two ImageFolders pointing at the same train dir so the val Subset uses the
...
# Class-balanced sampler — FER2013 disgust ≈ 436 vs happy ≈ 7215 makes uniform
# sampling underfit minority classes badly. RAF-DB imbalance is milder but the
# sampler still gives a small lift on F1.
...
def build_model() -> nn.Module:
    """ImageNet-pretrained ResNet with dropout + linear head."""
...
def evaluate_tencrop() -> float:
    ...
    # num_workers=0 because the closure above isn't picklable across procs.
